...
pub const BALL_VELOCITY_X: f32 = 35.0;
pub const BALL_VELOCITY_Y: f32 = 25.0;
pub const BALL_RADIUS: f32 = 2.0;


#[derive(Default)]
pub struct TCLNomiconState {
...
impl SimpleState for TCLNomiconState  {
    fn on_start(&mut self, data: StateData<'_, GameData<'_,'_>>) {
        ...
        //world.register::<Paddle>(); not neede anymore after having a system
        self.sprite_sheet_handle.replace(load_sprite_sheet(world));
        //initialise_ball(world, sprite_sheet_handle.clone().unwrap());
...
fn initialise_paddles(world: &mut World, sprite_sheet_handle: Handle<SpriteSheet>) {
    ...
    world.create_entity()
        ...
        .with(left_transform).build();
    world.create_entity()
        .with(sprite_render)
        .with(Paddle::new(Side::Right))
        .with(right_transform).build();
}
...
fn load_sprite_sheet(world: &mut World) -> Handle<SpriteSheet> {
    ...
    let texture_handle = {
        ...
        let texture_storage = world.read_resource::<AssetStorage
        <Texture>>();
        loader.load(
